...
import { readProgress, writeProgress } from "../../../db/progress";
...
export async function GET() {
  ...
  if (saved) {
    try { progress = JSON.parse(saved.payload); } catch { progress = null; }
  }
...
export async function PUT(request: Request) {
  ...
  if (!Number.isFinite(expectedUpdatedAt) || expectedUpdatedAt < 0) {
    return Response.json({ error: "Refresh before syncing progress." }, { status: 428 });
  }
  const payload = JSON.stringify(body.progress);
  if (payload.length > 1_000_000) return Response.json({ error: "Progress backup is too large." }, { status: 413 });
  const result = await writeProgress(user.userId, payload, expectedUpdatedAt);
